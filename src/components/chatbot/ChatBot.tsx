...
import { Zap, X, Send, User } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";
import { ChatMessage } from "./types";
import { getAIResponse } from "./chatFlows";
...
const ChatBot = () => {
  ...
  const handleButtonClick = async (label: string, action: string) => {
    ...
    switch (action) {
      case "ai_chatbots":
        ...
        await addBotMessage({
          id: Date.now().toString(), content: "Great choice! What would you like to know about our **AI Chatbot System**?",
      ...
      case "ai_voice":
        ...
        await addBotMessage({
          id: Date.now().toString(), content: "Awesome! What would you like to know about our **AI Voice Agent System**?",
          isUser: false, timestamp: new Date(),
          buttons: [
      ...
      case "chatbot_features":
        await addBotMessage({
          id: Date.now().toString(), content: "The full chatbot features are already listed on the website.\n\nPlease check the **AI Chatbot System** section to see all capabilities.\n\nWant to explore something else?",
          isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
        });
        setPhase("main");
        break;
      case "chatbot_results":
        await addBotMessage({
          id: Date.now().toString(), content: "Here's what businesses typically achieve with AI chatbots:\n\n• 24/7 instant customer replies\n• Faster lead capture\n• Reduced support workload\n• Higher response speed\n• Improved customer experience\n\nMany businesses see **better conversions** and **faster support resolution** after implementing automation.",
          isUser: false, timestamp: new Date(),
          buttons: [{ label: "Book a Call", action: "book_call" }, { label: "Back", action: "back_main" }],
        });
        setPhase("main");
        break;
      case "voice_features":
        await addBotMessage({
          id: Date.now().toString(), content: "The full AI Voice Agent features are listed on the website.\n\nPlease check the **AI Voice Agent System** section for the detailed capabilities.\n\nAnything else I can help with?",
          isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
        });
        setPhase("main");
        break;
      ...
      case "book_call":
        await addBotMessage({
          id: Date.now().toString(), content: "Want to discuss automation for your business?\n\nBook a free 45-minute consultation with our team.",
          isUser: false, timestamp: new Date(),
          buttons: [{ label: "Click here to book a call", action: "open_calendly" }, { label: "Back", action: "back_main" }],
        });
        break;
      case "open_calendly":
        window.open(CALENDLY_LINK, "_blank");
        await addBotMessage({
          id: Date.now().toString(), content: "Great. I've opened the booking page for you.\n\nPick a time that works best. Anything else?",
          isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
        });
        setPhase("main");
        break;
      case "talk_human":
        await addBotMessage({
          id: Date.now().toString(), content: "No problem. You can talk directly with us on WhatsApp.",
          isUser: false, timestamp: new Date(),
          buttons: [{ label: "Chat on WhatsApp", action: "open_whatsapp" }, { label: "Back", action: "back_main" }],
        });
        break;
      case "open_whatsapp":
        window.open(WHATSAPP_LINK, "_blank");
        await addBotMessage({
          id: Date.now().toString(), content: "I've opened WhatsApp for you.\n\nAnything else I can help with?",
          isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
        });
        setPhase("main");
        break;
      case "back_main":
        await addBotMessage({
          id: Date.now().toString(), content: `Sure${userName ? ` ${userName}` : ""}! What would you like to explore?`,
          isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
        });
        setPhase("main");
        break;
  ...
  const handleSendMessage = async () => {
    ...
    setMessages(prev => [...prev, {
      id: Date.now().toString(), content: userInput, isUser: true, timestamp: new Date(),
    }]);
    ...
    if (phase === "ask_name") {
      ...
      setPhase("main");
      await addBotMessage({
        id: Date.now().toString(), content: `Hello ${name}. How can I help you today?`,
        isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
      });
      return;
    }
    ...
    try {
      ...
      const contextMessage = history ? `Previous conversation:\n${history}\n\nUser: ${userInput}` : userInput;
      const aiResponse = await getAIResponse(contextMessage);
      setIsTyping(false);
      setMessages(prev => [...prev, {
        id: Date.now().toString(), content: aiResponse, isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
      }]);
    } catch {
      setIsTyping(false);
      setMessages(prev => [...prev, {
        id: Date.now().toString(), content: "Sorry, I'm having a moment. Please try again or use the options below.",
        isUser: false, timestamp: new Date(), buttons: MAIN_BUTTONS,
      }]);
    }
  };
  ...
  return <>
    ...
    <button
      ...
      className={cn(
        ...
        "hover:scale-110 hover:shadow-[0_0_30px_rgba(139,92,246,0.5)]",
        isOpen && "scale-0"
      )}
    ...
    <div className={cn(
      ...
      "w-full md:w-[420px] h-[100dvh] md:h-[620px]",
      "flex flex-col rounded-none md:rounded-2xl shadow-2xl overflow-hidden",
      ...
      <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] bg-violet-50 text-violet-600 px-2 py-1 rounded-full font-semibold border border-violet-100">AI Powered</span>
          <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="text-gray-400 hover:text-gray-600 h-8 w-8">
            <X className="h-5 w-5" />
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50/50 to-white">
        {messages.map((message) => (
          <div key={message.id} className={`flex gap-2.5 ${message.isUser ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            {!message.isUser && (
            ...
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
              message.isUser
                ? "bg-violet-600 text-white font-medium rounded-br-md shadow-md"
      ...
      {lastMessage?.buttons && !isTyping && (
        <div className="px-4 py-3 flex flex-wrap gap-2 border-t border-gray-100 bg-gray-50/50">
          {lastMessage.buttons.map((button, index) => (
            <button key={index} onClick={() => handleButtonClick(button.label, button.action)}
